...
void Demo::onInit() {
    ...
    if (modelNameArray.size() == 0) {
        ...
        prompt("MD2 Demo Error",
               "No MD2 models found in data/players.  (Download some from polycount.com and expand them into the data directory)",
               choice, true);
        app->endProgram = true;
        return;
    }
...
void Demo::onGraphics(RenderDevice *rd) {
    ...
    const double my = model->pose(CoordinateFrame(),
                                  MD2Model::Pose(MD2Model::STAND))->objectSpaceBoundingBox().getCorner(0).y;
    const double wy = weapon->pose(CoordinateFrame(),
                                   MD2Model::Pose(MD2Model::STAND))->objectSpaceBoundingBox().getCorner(0).y;
    ...
    double y = 10;
    double f = 16;
    app->debugFont->draw2D(format("%d fps", iRound(app->renderDevice->getFrameRate())), Vector2(x, y), 20,
                           Color3::yellow(), Color3::black());
    y += 30;
    app->debugFont->draw2D(format("%d characters", n), Vector2(x, y), f, Color3::cyan(), Color3::black());
    y += f * 1.5;
    app->debugFont->draw2D(format("%1.1f MB", model->mainMemorySize() / 1e6), Vector2(x, y), f, Color3::cyan(),
                           Color3::black());
    y += f * 1.5;
    app->debugFont->draw2D(format("%1.0f Mtris/sec", app->renderDevice->getTriangleRate() / 1e6), Vector2(x, y), f,
                           Color3::cyan(), Color3::black());
    y += f * 1.5;

    app->debugFont->draw2D(model->name(),
                           Vector2(app->renderDevice->getWidth() / 2, app->renderDevice->getHeight() - 45), 30,
                           Color3::black(), Color3::white(), GFont::XALIGN_CENTER);

    x = app->renderDevice->getWidth() - 130;
    y = 10;
    f = 12;
    app->debugFont->draw2D("CLICK   attack", Vector2(x, y), f, Color3::cyan(), Color3::black());
    y += f * 1.5;
    app->debugFont->draw2D("SPACE  jump", Vector2(x, y), f, Color3::cyan(), Color3::black());
    y += f * 1.5;
    app->debugFont->draw2D("CTRL     crouch", Vector2(x, y), f, Color3::cyan(), Color3::black());
    y += f * 1.5;
    app->debugFont->draw2D("1 . . 5    taunt", Vector2(x, y), f, Color3::cyan(), Color3::black());
    y += f * 1.5;
    app->debugFont->draw2D("6 . . 8    die", Vector2(x, y), f, Color3::cyan(), Color3::black());
    y += f * 1.5;
    app->debugFont->draw2D("9 . . -    pain", Vector2(x, y), f, Color3::cyan(), Color3::black());
    y += f * 1.5;
    app->debugFont->draw2D("R/T       run/back", Vector2(x, y), f, Color3::cyan(), Color3::black());
    y += f * 1.5;
    app->debugFont->draw2D("e           new character", Vector2(x, y), f, Color3::cyan(), Color3::black());
    y += f * 1.5;
...
void Demo::loadSkins(const std::string &dir, double brighten) {
    ...
    for (int i = 0; i < textureName.size(); ++i) {
            ...
            if (GImage::supportedFormat(ext)) {
                ...
                modelTexture.append(
                        Texture::fromFile(filename, TextureFormat::AUTO, Texture::TILE, Texture::TRILINEAR_MIPMAP,
                                          Texture::DIM_2D, brighten));
            }
        }
    ...
    if (weapon->textureFilenames().size() > 0) {
        ...
        if (fileExists(filename)) {
            weaponTexture = Texture::fromFile(filename, TextureFormat::AUTO, Texture::TILE, Texture::TRILINEAR_MIPMAP,
                                              Texture::DIM_2D, brighten);
        } else {
            filename = dir + "weapon.pcx";
            if (fileExists(filename)) {
                weaponTexture = Texture::fromFile(filename, TextureFormat::AUTO, Texture::TILE,
                                                  Texture::TRILINEAR_MIPMAP, Texture::DIM_2D, brighten);
            }
        }
    } else {
        weaponTexture = NULL;
