...
void Scene::generateShadowMap(
    ...
    app->renderDevice->setProjectionMatrix(
...
GameTime Scene::timeUntilCollisionWithMovingSphere(
    ...
    for (int i = 0; i < object.size(); ++i) {
            ...
            testTime = object[i]->timeUntilCollisionWithMovingSphere(sphere, velocity, timeLimit, testLocation,
                                                                     testNormal);
...
void Scene::simulate(GameTime duration) {
    ...
    for (int i = 0; i < sim.size(); ++i) {
            ...
            while ((time < duration) && (count < 10)) {
                ...
                delta = timeUntilCollisionWithMovingSphere(obj->sphere, obj->velocity, duration, collisionLocation,
                                                           collisionNormal);
                ...
                if (fuzzyEq(delta, 0.0)) {
                    ...
                    if ((collisionLocation - obj->sphere.center).length() >
                        ...
                        Sphere bumpedSphere(obj->sphere.center + collisionNormal * restingInterpenetration,
                                            obj->sphere.radius);
                        delta = timeUntilCollisionWithMovingSphere(bumpedSphere, obj->velocity, duration,
                                                                   collisionLocation, collisionNormal);
                ...
                if (fuzzyLt(time + delta, duration)) {
                    ...
                    Vector3 bounceDirection =
                            CollisionDetection::bounceDirection(obj->sphere, obj->velocity, delta, collisionLocation,
                                                                collisionNormal);
                    ...
                    if (RESTING_CONTACT && obj->velocity.isZero() && !acceleration.isZero()) {
                        GameTime t = timeUntilCollisionWithMovingSphere(obj->sphere, acceleration * duration, duration,
                                                                        collisionLocation, collisionNormal);
                        // See if acceleration will cause us to leave resting contact
                        if (t < 0.01) {
                            // If we accelerate, we'll immediately hit a surface
                            Vector3 bounceDirection =
                                    CollisionDetection::bounceDirection(obj->sphere, acceleration * duration, t,
                                                                        collisionLocation, collisionNormal);
