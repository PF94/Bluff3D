...
Rect2D Report::drawPopup(const char *title) {
    ...
    float s = w * 0.013;
    app->titleFont->draw2D(app->renderDevice, title, Vector2(rect.x0() + 4, rect.y0()), s * 1.5, Color3::black(),
                           Color4::clear(), GFont::XALIGN_LEFT, GFont::YALIGN_TOP);
...
#       define PRINT(str) p.y += app->reportFont->draw2D(app->renderDevice, str, p, s, Color3::black()).y
    ...
    p.y += s * 2;
    LABEL("Extensions");
    PRINT(std::string("FSAA:                           ") +
          ((GLCaps::supports("WGL_ARB_multisample") || GLCaps::supports("GL_ARB_multisample")) ? "Yes" : "No"));
    ...
    PRINT(std::string("Vertex Arrays:              ") +
          (GLCaps::supports_GL_ARB_vertex_buffer_object() ? "Yes" : "No"));
...
#       endif
    ...
    p.y += app->reportFont->draw2D(app->renderDevice, format("f%d", app->featureRating), Vector2(x0, p.y), s * 2,
                                   Color3::red() * 0.5).y;
    ...
    performanceButton = Rect2D::xywh(p,
                                     app->titleFont->draw2D(app->renderDevice, "Speed", p - Vector2(w * 0.0075f, 0),
                                                            s * 2, Color3::white() * 0.4f));

    p.y += app->reportFont->draw2D(app->renderDevice, format("%5.1f", iRound(app->performanceRating * 10) / 10.0f),
                                   Vector2(x0 - s * 2, p.y), s * 2, Color3::red() * 0.5).y;
    ...
    p.y += app->reportFont->draw2D(app->renderDevice, quality(app->bugCount), Vector2(x0, p.y), s * 2,
                                   Color3::red() * 0.5f).y;
...
#               define PRINT(cap, val)   \
            ...
            app->reportFont->draw2D(app->renderDevice, "Incoherent", p + Vector2(220, 0), s, Color3::black(),
                                    Color4::clear(), GFont::XALIGN_RIGHT);
            p.y += app->reportFont->draw2D(app->renderDevice, "Coherent", p + Vector2(350, 0), s, Color3::black(),
                                           Color4::clear(), GFont::XALIGN_RIGHT).y;

            app->reportFont->draw2D(app->renderDevice, "FPS*   MVerts/s", p + Vector2(240, 0), s, Color3::black(),
                                    Color4::clear(), GFont::XALIGN_RIGHT);
            p.y += app->reportFont->draw2D(app->renderDevice, "FPS*   MVerts/s", p + Vector2(370, 0), s,
                                           Color3::black(), Color4::clear(), GFont::XALIGN_RIGHT).y;
            ...
            app->reportFont->draw2D(app->renderDevice, (app->vertexPerformance.drawArraysVBOPeakFPS > 0) ? \
                    ...
                    std::string("X"), p + Vector2(330, 0), s, Color3::red() * 0.5, Color4::clear(),
                                    GFont::XALIGN_RIGHT);\
                p.y += app->reportFont->draw2D(app->renderDevice, (app->vertexPerformance.drawArraysVBOPeakFPS > 0) ? \
                    ...
                    std::string("X"), p + Vector2(380, 0), s, Color3::red() * 0.5, Color4::clear(),
                                               GFont::XALIGN_RIGHT).y;

#               undef PRINT

            p.y += s;
            p.y += app->reportFont->draw2D(app->renderDevice, format("* FPS at %d k polys/frame.",
                                                                     iRound(app->vertexPerformance.numTris / 1000.0)),
                                           p + Vector2(20, 0), s, Color3::black()).y;
        }
    }
