...
namespace G3D {
    ...
    static std::string getTextureState(bool showDisabled) {
        ...
        if (!GLCaps::supports_GL_ARB_multitexture()) {
            ...
            bool enabled = false;
            for (int tt = 0; tt < 3; ++tt)enabled = enabled || (glGetBoolean(tname[tt]) ? true : false);

            if (!enabled && !showDisabled) {
                for (int tt = 0; tt < 3; ++tt)result += format("glDisable(%s); ", GLenumToString(tname[tt]));
                result += "\n";
            } else {
