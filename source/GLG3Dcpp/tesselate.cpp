                ...
                switch (primitive.primitiveType) {
                    case GL_TRIANGLES:
                        ...
                        for (i = 0; i < primitive.vertex.size(); i += 3) {
                                output.append(Triangle(primitive.vertex[i], primitive.vertex[i + 1],
                                                       primitive.vertex[i + 2]));
                            }
                        break;
                    ...
                    case GL_TRIANGLE_FAN: {
                        ...
                        for (i = 1; i < primitive.vertex.size() - 1; ++i) {
                                output.append(
                                        Triangle(primitive.vertex[0], primitive.vertex[i], primitive.vertex[i + 1]));
                            }
                    }
                        break;
                    ...
                    case GL_TRIANGLE_STRIP:
                        for (i = 0; i < primitive.vertex.size() - 3; i += 2) {
                                output.append(Triangle(primitive.vertex[i], primitive.vertex[i + 1],
                                                       primitive.vertex[i + 2]));
                                output.append(Triangle(primitive.vertex[i + 2], primitive.vertex[i + 1],
                                                       primitive.vertex[i + 3]));
                            }
                        if (i < primitive.vertex.size() - 2) {
                            output.append(
                                    Triangle(primitive.vertex[i], primitive.vertex[i + 1], primitive.vertex[i + 2]));
                        }
                        break;
