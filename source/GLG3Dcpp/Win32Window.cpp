...
#define WGL_SAMPLES_ARB            0x2042
    ...
    static unsigned int _sdlKeys[SDL_NUM_SCANCODES];
    ...
    void Win32Window::setInputCapture(bool c) {
        ...
        if (c != _inputCapture) {
            ...
            if (_inputCapture) {
                ...
                RECT rect =
                        {static_cast<LONG>(clientX + clientRectOffset.x),
                         static_cast<LONG>(clientY + clientRectOffset.y),
                         static_cast<LONG>(clientX + settings.width + clientRectOffset.x),
                         static_cast<LONG>(clientY + settings.height + clientRectOffset.y)};
                ClipCursor(&rect);
            } else {
                ClipCursor(NULL);
            }
...
/** Changes the screen resolution */
    ...
    static void makeKeyEvent(int vkCode, int lParam, GEvent &e) {
        ...
        if ((vkCode >= 'A') && (vkCode <= 'Z')) {
            ...
            e.key.keysym.sym = (vkCode - 'A' + 'a');

        } else if (vkCode == VK_SHIFT) {

            e.key.keysym.sym = (extended ? SDLK_RSHIFT : SDLK_LSHIFT);

        } else if (vkCode == VK_CONTROL) {

            e.key.keysym.sym = (extended ? SDLK_RCTRL : SDLK_LCTRL);

        } else if (vkCode == VK_MENU) {

            e.key.keysym.sym = (extended ? SDLK_RALT : SDLK_LALT);

        } else {

            e.key.keysym.sym = _sdlKeys[iClamp(vkCode, 0, SDL_NUM_SCANCODES)];

        }

        // seems REALLY shitty
        e.key.keysym.scancode = static_cast<SDL_Scancode>(MapVirtualKey(vkCode, 0));
        //(lParam >> 16) & 0x7F;
 ...
 */
    static void mouseButton(bool down, int keyEvent, DWORD flags, GEvent &e) {
        ...
        e.key.keysym.sym = keyEvent;

        e.key.keysym.scancode = static_cast<SDL_Scancode>(0);
 ...
 */
    static void initWin32KeyMap() {
        ...
        _sdlKeys[VK_NUMPAD0] = SDLK_KP_0;
        _sdlKeys[VK_NUMPAD1] = SDLK_KP_1;
        _sdlKeys[VK_NUMPAD2] = SDLK_KP_2;
        _sdlKeys[VK_NUMPAD3] = SDLK_KP_3;
        _sdlKeys[VK_NUMPAD4] = SDLK_KP_4;
        _sdlKeys[VK_NUMPAD5] = SDLK_KP_5;
        _sdlKeys[VK_NUMPAD6] = SDLK_KP_6;
        _sdlKeys[VK_NUMPAD7] = SDLK_KP_7;
        _sdlKeys[VK_NUMPAD8] = SDLK_KP_8;
        _sdlKeys[VK_NUMPAD9] = SDLK_KP_9;
        ...
        _sdlKeys[VK_NUMLOCK] = SDLK_NUMLOCKCLEAR;
        _sdlKeys[VK_CAPITAL] = SDLK_CAPSLOCK;
        _sdlKeys[VK_SCROLL] = SDLK_SCROLLLOCK;
        ...
        _sdlKeys[VK_RWIN] = SDLK_RGUI;
        _sdlKeys[VK_LWIN] = SDLK_LGUI;

        _sdlKeys[VK_HELP] = SDLK_HELP;
        _sdlKeys[VK_PRINT] = SDLK_PRINTSCREEN;
        _sdlKeys[VK_SNAPSHOT] = SDLK_PRINTSCREEN;
        _sdlKeys[VK_CANCEL] = SDLK_PAUSE;
