            ...
            case GL_TEXTURE_RECTANGLE_EXT:
                ...
                if (compressed) {
                    ...
                    glCompressedTexImage2DARB(target, mipLevel, bytesActualFormat, width,
                                              height, 0, (bytesPerPixel * ((width + 3) / 4) * ((height + 3) / 4)),
                                              rawBytes);
                    ...
                    glTexImage2D(target, mipLevel, textureFormat, width, height, 0, bytesFormat, GL_UNSIGNED_BYTE,
                                 bytes);
                }
                break;
...
/////////////////////////////////////////////////////////////////////////////
    ...
    void Texture::getImage(GImage &dst, const TextureFormat *outFormat) const {
        ...
        switch (outFormat->OpenGLBaseFormat) {
            ...
            default: alwaysAssertM(false, "This texture format is not appropriate for reading to an image.");
        }
    ...
    TextureRef Texture::fromMemory(
        ...
        return Texture::fromMemory(name, b, bytesFormat, width, height, 1,
                                   desiredFormat, param.wrapMode, param.interpolateMode, dimension, param.depthReadMode,
                                   param.maxAnisotropy);
    }
        ...
        if (G3D::toUpper(ddsExt) == "DDS") {
            ...
            for (int i = 0; i < numMipMaps; ++i) {
                    ...
                    for (int face = 0; face < numFaces; ++face) {
                            ...
                            byteStart += ((bytesFormat->packedBitsPerTexel / 8) * ((mapWidth + 3) / 4) *
                                          ((mapHeight + 3) / 4));
                        }
            ...
            return Texture::fromMemory(filename[0], byteMipMapFaces, bytesFormat, ddsTexture.getWidth(),
                                       ddsTexture.getHeight(), 1, desiredFormat, wrap, interpolate, dimension,
        ...
        for (int mipLevel = 0; mipLevel < numMipMaps; ++mipLevel) {
                ...
                for (int f = 0; f < numFaces; ++f) {
                        ...
                        if (isMipMapformat(interpolate) && !hasAutoMipMap() && (numMipMaps == 1)) {
                            ...
                            createTexture(target, reinterpret_cast<const uint8 *>(bytes[mipLevel][f]),
                                          bytesFormat->OpenGLBaseFormat,
