...
// Cached between render calls
    ...
    IFSModelRef
    IFSModel::create(const std::string &filename, double scale, const CoordinateFrame &cframe, const bool weld) {
    ...
    void
    IFSModel::load(const std::string &filename, const Vector3 &scale, const CoordinateFrame &cframe, const bool weld) {
        reset();
    ...
    size_t IFSModel::mainMemorySize() const {
        ...
        size_t frameSize =
                sizeof(MeshAlg::Geometry) + (sizeof(Vector3) + sizeof(Vector3)) * geometry.vertexArray.size();
    ...
    void IFSModel::save(
        ...
        if ("ifs" == filenameExt(filename)) {
            ...
            if (ifs_version == 1.1f) {
                ...
                alwaysAssertM(texCoord.size() == vertex.size(),
                              "Number of texCoords must match the number of vertices");
            ...
            alwaysAssertM(texCoord.size() == 0,
                          format("texCoord.size() != 0, PLY2 files do not support saving texCoords."));
