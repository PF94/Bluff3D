...
namespace G3D {
    ...
    size_t MD2Model::mainMemorySize() const {
        ...
        size_t frameSize = keyFrame.size() * (sizeof(PackedGeometry) +
                                              (sizeof(Vector3) + sizeof(uint8)) * keyFrame[0].vertexArray.size());
    ...
    void MD2Model::getGeometry(const Pose &pose, MeshAlg::Geometry &out) const {
        ...
        if ((interpolatedModel == this) && (pose == interpolatedPose)) {
            ...
            if (&out != &interpolatedFrame) {
                // Copy from the cache
                System::memcpy(out.vertexArray.getCArray(), interpolatedFrame.vertexArray.getCArray(),
                               sizeof(Vector3) * interpolatedFrame.vertexArray.size());
                System::memcpy(out.normalArray.getCArray(), interpolatedFrame.normalArray.getCArray(),
                               sizeof(Vector3) * interpolatedFrame.vertexArray.size());
            }
