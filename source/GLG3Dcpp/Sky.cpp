    ...
    SkyRef Sky::fromFile(
        ...
        if (GLCaps::supports_GL_ARB_texture_cube_map() && !GLCaps::hasBug_normalMapTexGen()) {
            ...
            if (_filename[1] == "") {
                ...
                faceTextures[0] = Texture::fromFile(filenameBase + "*" + filenameExt, format, Texture::CLAMP,
                                                    Texture::TRILINEAR_MIPMAP, Texture::DIM_CUBE_MAP, 1.0,
                                                    Texture::DEPTH_NORMAL, 1.0, 1.0f / scaleDownFactor);
            } else {
                // Specified six filenames
                faceTextures[0] = Texture::fromFile(_filename, format, Texture::CLAMP, Texture::TRILINEAR_MIPMAP,
                                                    Texture::DIM_CUBE_MAP, 1.0, Texture::DEPTH_NORMAL, 1.0,
                                                    1.0f / scaleDownFactor);
            }
            ...
            if (_filename[1] == "") {
                ...
                for (int t = 0; t < 6; ++t) {
                        faceTextures[t] = Texture::fromFile(filenameBase + ext[t] + filenameExt,
                                                            format, Texture::CLAMP, Texture::BILINEAR_NO_MIPMAP,
                                                            Texture::DIM_2D,
    ...
    Sky::Sky(
        ...
        if (drawCelestialBodies) {
            moon = Texture::fromTwoFiles(directory + "moon.jpg", directory + "moon-alpha.jpg",
                                         alphaFormat, Texture::TRANSPARENT_BORDER, Texture::BILINEAR_NO_MIPMAP,
                                         Texture::DIM_2D,
                                         Texture::DEPTH_NORMAL, 1.0);

            sun = Texture::fromFile(directory + "sun.jpg", format, Texture::TRANSPARENT_BORDER,
                                    Texture::BILINEAR_NO_MIPMAP, Texture::DIM_2D, 1.0, Texture::DEPTH_NORMAL, 1.0);
            disk = Texture::fromFile(directory + "lensflare.jpg", format, Texture::TRANSPARENT_BORDER,
                                     Texture::BILINEAR_NO_MIPMAP, Texture::DIM_2D, 1.0, Texture::DEPTH_NORMAL, 1.0);
            sunRays = Texture::fromFile(directory + "sun-rays.jpg", format, Texture::TRANSPARENT_BORDER,
                                        Texture::BILINEAR_NO_MIPMAP, Texture::DIM_2D, 1.0, Texture::DEPTH_NORMAL, 1.0);
 ...
 */
    ...
    void Sky::drawMoonAndStars(
        ...
        if (lighting.moonPosition.y > -0.3f) {
            ...
            renderDevice->pushState();
            // Rotate stars
            renderDevice->setObjectToWorldMatrix(
                    (lighting.physicallyCorrect ? lighting.trueStarFrame : lighting.starFrame));
        ...
        drawCelestialSphere(renderDevice, L, X, Y, .06,
                            Color4(lighting.emissiveScale, min(1.0f, max(0.0f, moonPosition.y * 4.0f))));
    }
    ...
    void Sky::renderLensFlare(
        ...
        if (sunPosition.dot(camera.getLookVector()) > 0) {
            ...
            if (fractionOfSunVisible > 0.0) {
                ...
                if ((sunPosition.x > 0) &&
                    ...
                    Color4 col =
                            Color4(1, 1, 1, 1) * (occlusionAttenuation *
                                                  0.4f *
                                                  max(0.0f, min(1.0f, 1.0f - sunPosition.y * 2.0f / sqrtf(2.0f))));
                ...
                static const Color3 color[] = {Color3(6, 4, 0) / 255, Color3(6, 4, 0) / 255, Color3(0, 12, 0) / 255,
                                               Color3(0, 12, 0) / 255, Color3(0, 12, 0) / 255, Color3(0, 12, 0) / 255,
                                               Color3(10, 0, 0) / 255, Color3(0, 12, 0) / 255, Color3(10, 0, 0) / 255,
                                               Color3::fromARGB(0x192125) / 10, Color3::fromARGB(0x1F2B1D) / 10,
                                               Color3::fromARGB(0x1F2B1D) / 10};
                int numFlares = 12;
