...
namespace G3D {
    ...
    void Draw::lineSegment(
        ...
        Vector3 eye = renderDevice->getObjectToWorldMatrix().pointToObjectSpace(
                renderDevice->getCameraToWorldMatrix().translation);
        Vector3 E = eye - v0;
        Vector3 V = v1 - v0;
        Vector3 U = E.cross(V);
    ...
    void Draw::fullScreenImage(const GImage &im, RenderDevice *renderDevice) {
        ...
        glDrawPixels(im.width, im.height, (im.channels == 3) ? GL_RGB : GL_RGBA, GL_UNSIGNED_BYTE,
                     (const GLvoid *) im.byte());
