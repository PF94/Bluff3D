...
static bool SDL_handleErrorCheck_(
    ...
    SDLWindow::SDLWindow(const GWindowSettings &settings) {
        ...
        int flags =
                ...
                (settings.framed ? 0 : SDL_WINDOW_BORDERLESS);
...
#	endif
    ...
    bool SDLWindow::hasFocus() const {
        uint8 s = SDL_GetWindowFlags(this->window);

        return ((s & SDL_WINDOW_MOUSE_FOCUS) != 0) &&
               ((s & SDL_WINDOW_INPUT_FOCUS) != 0);
    }
    ...
    std::string SDLWindow::joystickName(unsigned int sticknum) {
        ...
        return SDL_JoystickName(reinterpret_cast<SDL_Joystick *>(sticknum));
    }
    ...
    void SDLWindow::setRelativeMousePosition(double x, double y) {
        SDL_WarpMouseInWindow(this->window, iRound(x), iRound(y));
    }
