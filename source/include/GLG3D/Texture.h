 ...
 */
    ...
    public:
        ...
        enum Dimension {
            DIM_2D = 2, DIM_2D_RECT = 4, DIM_CUBE_MAP = 5, DIM_2D_NPOT = 6, DIM_CUBE_MAP_NPOT = 7
        };

        /** TRANSPARENT_BORDER provides a border of Color4(0,0,0,0) and clamps to it. */
        enum WrapMode {
            TILE = 1, CLAMP = 0, TRANSPARENT_BORDER = 2
        };
         ...
         */
        enum DepthReadMode {
            DEPTH_NORMAL = 0, DEPTH_LEQUAL = 1, DEPTH_GEQUAL = 2
        };
    ...
    public:
        ...
        */
        ...
        inline static TextureRef fromGImage(
                ...
                Dimension dimension = DIM_2D,
                const Parameters &param = Parameters::defaults()) {
            return fromGImage(name, image, desiredFormat, param.wrapMode, param.interpolateMode, dimension,
                              param.depthReadMode, param.maxAnisotropy);
        }
