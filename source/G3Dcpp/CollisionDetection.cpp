...
#   pragma warning (disable : 4244)
    ...
    float CollisionDetection::collisionTimeForMovingSphereFixedSphere(
        ...
        double time = collisionTimeForMovingPointFixedSphere(movingSphere.center, velocity, Sphere(fixedSphere.center,
                                                                                                   fixedSphere.radius +
                                                                                                   movingSphere.radius),
                                                             location, outNormal);
...
*/
    ...
    float CollisionDetection::collisionTimeForMovingPointFixedBox(
        ...
        for (f = 1; f < 6; ++f) {
                Vector3 pos;
                box.getFaceCorners(f, v[0], v[1], v[2], v[3]);
                float time = collisionTimeForMovingPointFixedRectangle(point, velocity, v[0], v[1], v[2], v[3], pos,
                                                                       normal);
                if (time < bestTime) {
                    bestTime = time;
                    outNormal = normal;
...
/** Used by collisionTimeForMovingPointFixedCapsule.
	...
	*/
    ...
    float CollisionDetection::collisionTimeForMovingPointFixedCapsule(
        ...
        findRayCapsuleIntersection(Ray::fromOriginAndDirection(point, direction), capsule, numIntersections,
                                   intersection);
    ...
    float CollisionDetection::collisionTimeForMovingSphereFixedTriangle(
        ...
        if (isPointInsideTriangle(triangle.vertex(0), triangle.vertex(1), triangle.vertex(2), triangle.normal(),
                                  outLocation, triangle.primaryAxis())) {
        ...
        Vector3 point = closestPointToTrianglePerimeter(triangle._vertex, triangle.edgeDirection,
                                                        triangle.edgeMagnitude, outLocation);
    ...
    float CollisionDetection::collisionTimeForMovingSphereFixedBox(
        ...
        int f = 0;
        box.getFaceCorners(f, v[0], v[1], v[2], v[3]);
        bestTime = collisionTimeForMovingSphereFixedRectangle(sphere, velocity, v[0], v[1], v[2], v[3], location,
                                                              outNormal);

        for (f = 1; f < 6; ++f) {
                Vector3 pos, normal;
                box.getFaceCorners(f, v[0], v[1], v[2], v[3]);
                float time = collisionTimeForMovingSphereFixedRectangle(sphere, velocity, v[0], v[1], v[2], v[3], pos,
                                                                        normal);
                if (time < bestTime) {
                    bestTime = time;
                    location = pos;
