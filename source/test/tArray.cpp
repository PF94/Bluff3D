...
void perfArray() {
...

    {
        ...
        printf("  G3D::Array               %9.02f     %9.02f     %s\n",
               ...
               G3Dwin ? " ok " : "FAIL");
        printf("  std::vector              %9.02f     %9.02f\n", (double) vectorResizeBig / M,
               (double) vectorResizeSmall / M);
        printf("  realloc(*)               %9.02f     %9.02f\n", (double) mallocResizeBig / M,
               (double) mallocResizeSmall / M);
...
#undef LOOPS

    {
        ...
        printf("  G3D::Array                 %5.02f    %5.02f   %5.02f\n", (arrayAllocInt / (float) size),
               (arrayAccessInt / (float) (N * size)), (arrayFreeInt / (float) size));
        printf("  std::vector                %5.02f    %5.02f   %5.02f\n", (vectorAllocInt / (float) size),
               (vectorAccessInt / (float) (N * size)), (vectorFreeInt / (float) size));
        printf("  new/delete                 %5.02f    %5.02f   %5.02f\n", (newAllocInt / (float) size),
               (newAccessInt / (float) (N * size)), (newFreeInt / (float) size));
        printf("  malloc/free                %5.02f    %5.02f   %5.02f\n", (mallocAllocInt / (float) size),
               (mallocAccessInt / (float) (N * size)), (mallocFreeInt / (float) size));
        printf("  System::alignedMalloc      %5.02f    %5.02f   %5.02f\n", (sysmallocAllocInt / (float) size),
               (sysmallocAccessInt / (float) (N * size)), (sysmallocFreeInt / (float) size));
        printf("\n");
    }
...
#   undef LOOPS
...

    {
        ...
        printf("                             Alloc    Access   Free\n");
        printf("  G3D::Array               %7.02f    %5.02f   %5.02f\n", (arrayAllocBig / (float) size),
               (arrayAccessBig / (float) (N * size)), (arrayFreeBig / (float) size));
        printf("  std::vector              %7.02f    %5.02f   %5.02f\n", (vectorAllocBig / (float) size),
               (vectorAccessBig / (float) (N * size)), (vectorFreeBig / (float) size));
        printf("  new/delete               %7.02f    %5.02f   %5.02f\n", (newAllocBig / (float) size),
               (newAccessBig / (float) (N * size)), (newFreeBig / (float) size));
        printf("  malloc/free(*)           %7.02f    %5.02f   %5.02f\n", (mallocAllocBig / (float) size),
               (mallocAccessBig / (float) (N * size)), (mallocFreeBig / (float) size));
        printf("  System::alignedMalloc(*) %7.02f    %5.02f   %5.02f\n", (sysmallocAllocBig / (float) size),
               (sysmallocAccessBig / (float) (N * size)), (sysmallocFreeBig / (float) size));
        printf("    * does not call constructor or destructor!\n\n");
    }
