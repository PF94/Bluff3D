...
*/
    ...
    void GConsole::completeCommand(int direction) {
        ...
        m_completionArrayIndex =
                (m_completionArrayIndex + m_completionArray.size() + direction) % m_completionArray.size();
    ...
    void GConsole::render(RenderDevice *rd) {
        ...
        for (int count = 0; count < m_settings.numVisibleLines - 1; ++count) {
                ...
                if (q >= 0) {
                    m_font->draw2D(rd, m_buffer[q].value, rect.x0y1() - Vector2(0, m_settings.lineHeight * (count + 2)),
                                   fontSize, m_buffer[q].color);
                }
            }
        ...
        m_font->draw2D(rd, m_currentLine, rect.x0y1() - Vector2(0, m_settings.lineHeight), fontSize,
                       m_settings.defaultCommandColor);
        ...
        if (solidCursor) {
            ...
            m_font->draw2D(rd, "_", rect.x0y1() + Vector2(bounds.x, -m_settings.lineHeight), fontSize,
                           m_settings.defaultCommandColor);
        }
