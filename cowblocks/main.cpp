/**
 @file cowblocks/main.cpp

 @maintainer space, jeevacation2331@gmail.com
 @created 2002-10-22
 @edited  2026-05-31
 */
 ...
 */
const std::string modelName = "cow.ifs";

enum RenderMethod {
    TRIANGLES = 0, VARSTREAM, VARSTATIC, NUM_RENDER_METHODS
} renderMethod;
...
void Demo::onInit() {
    ...
    model = new Model(app->dataDir + "models/" + modelName, varStatic);
    gameTime = G3D::toSeconds(8, 00, 00, AM);
    renderMethod = VARSTREAM;
...
void Demo::onGraphics(RenderDevice *rd) {
    ...
    app->renderDevice->setShininess(24);
    debugAssertGLOk();
    ...
    static const Color3 color[] = {Color3::red(), Color3::purple(), Color3::brown(), Color3::black()};
    double t[2];
    Vector3 c[2];
    ...
    char *cow1 = "Cow1";
    char *cow2 = "Timer: ";
    std::string tStr = std::to_string(System::getTick());
    // switch (renderMethod) {
 //     case VARSTREAM:
   //        str = "Test";
        //   break;
 ...
 //     default:;
  //  }
    ...
    app->debugFont->draw2D(cow1, Vector2(1110, app->renderDevice->getHeight() - 710), 30, Color3::red(),
                           Color3::black());

    app->debugFont->draw2D(cow2, Vector2(1026, app->renderDevice->getHeight() - 670), 30, Color3::yellow(),
                           Color3::black());

    app->debugFont->draw2D(tStr, Vector2(1110, app->renderDevice->getHeight() - 670), 30, Color3::yellow(),
                           Color3::black());
...
App::App(const GAppSettings &settings) : GApp(settings) {
    renderDevice->setCaption("funny cow game");
}
...
int main(int argc, char **argv) {
  ...
  settings.debugFontName = "content/fonts/dominant.fnt";
